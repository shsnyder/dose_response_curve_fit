...
def find_skewnorm_curve_parameters(func, x, y):
    try:
        init_loc = (x[0]+x[-1])/2
        popt, pcov, infodict, mesg, ier = curve_fit(func, x, y, method='trf', full_output=True, 
                                                    p0=[0,init_loc,1],
...
def analyze_dose_response(dose, response):
    ...
    # Curve fit
    a, loc, scale, ier, mesg = find_skewnorm_curve_parameters(func_skewnorm_cdf, dose, trans_response)
...
def successful_skewnorm_curvefit(a, loc, scale, ier):
    ...
    if reported_curvefilt_failure:
        ...
        failed_curve_fit  = ((a, loc, scale) == (0, 0, 0)) or ((a, loc, scale) == (1, 1, 1))
