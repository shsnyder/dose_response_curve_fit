...
def plot_skewnorm(x, y,  a, loc, scale, plot_file=None):
    ...
    x1 = np.linspace(skewnorm.ppf(0.00001, a, loc, scale), 
                     skewnorm.ppf(0.999, a, loc, scale), 
                     100)
    sns.lineplot(x='dose_pred', y='response_pred', 
                    # data=pd.DataFrame({'dose_pred':x1, 
                    data=pd.DataFrame({'dose_pred':x1, 
                                     'response_pred':skewnorm.cdf(x1, a, loc, scale)}),
                    color='blue', lw=5, alpha=0.6, 
...
def plot_skewnorm_test(x, y,  a, loc, scale, plot_file=None):
    # Plot original data
    ...
    sns.lineplot(x='dose_pred', y='response_pred', 
                    # data=pd.DataFrame({'dose_pred':x1, 
                    data=pd.DataFrame({'dose_pred':dose_pred, 
                                     'response_pred':skewnorm.cdf(x1, a, loc, scale)}),
                    color='blue', lw=5, alpha=0.6, 
                    label='Skewnormal CDF fit')
    plt.xscale('log')
...
def plot_skewnorm_curve_fit(x, y,  a, loc, scale, plot_file=None):
    ...
    ax = sns.scatterplot(x='Dose', y='Response', color='red', data=dose_response, label='Response data')
    ...
    x1 = np.linspace(skewnorm.ppf(0.00001, a, loc, scale), 
                     ...
                     100)
    sns.lineplot(x='dose_pred', y='response_pred', 
                    data=pd.DataFrame({'dose_pred':np.power(10, x1), 
                                     'response_pred':data_processing.restore_response(skewnorm.cdf(x1, a, loc, scale), start, range)}),
                    color='blue', lw=5, alpha=0.6, 
                    label='Skewnormal CDF fit')
    ax.text(0.05, 0.4, f'a: {a:.3f}\nloc: {loc:.3f}\nscale:{scale:.3f}', 
            ha="left", va="bottom",
...
def plot_hill_eqn(hill_coef, ac50):
    ...
    x1 = np.linspace(data_processing.inv_hill_eqn(fit_eqn_limit_lower, hill_coef, ac50), 
                     data_processing.inv_hill_eqn(fit_eqn_limit_upper, hill_coef, ac50), 
                     100)

    hill_data =  data_processing.hill_eqn(x1, hill_coef, ac50)

    ax = sns.lineplot(x='dose_pred', y='response_pred', 
                        data=pd.DataFrame({'dose_pred':x1, 
                                            'response_pred': hill_data} ),
                        color='green', lw=5, alpha=0.6, 
                        label='Hill equation')
...
def plot_skewnorm_hill(x, y,  a, loc, scale, hill_coef, ac50, plot_file=None):
    ...
    x1 = np.linspace(skewnorm.ppf(fit_eqn_limit_lower, a, loc, scale), 
                     skewnorm.ppf(fit_eqn_limit_upper, a, loc, scale), 
                     100)

    x2 = np.logspace(np.log10(data_processing.inv_hill_eqn(fit_eqn_limit_lower_hill, hill_coef, ac50)), 
                     np.log10(data_processing.inv_hill_eqn(fit_eqn_limit_upper_hill, hill_coef, ac50)), 
                     100)
    ...
    sns.lineplot(x='dose_pred', y='response_pred', 
                    data=pd.DataFrame({'dose_pred':x2, 
                                     'response_pred': transformed_hill_data}),
                    color='green', lw=5, alpha=0.6, 
