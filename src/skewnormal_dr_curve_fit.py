...
def read_tox21_data(data_file, curve_class_limit=None):
    ...
    if curve_class_limit:
        dr_filtered = dr_filtered[
            np.abs(dr_filtered["CURVE_CLASS2"]) < curve_class_limit
        ]
...
if __name__ == "__main__":
    ...
    for index, row in dr_filtered.iterrows():
        ...
        (
            ...
            row, response_cols, dose_cols, logdose=True
        )
        ...
        data_summary["sample_data_id"] = row["SAMPLE_DATA_ID"]
        data_summary["sample_name"] = sample_name
        data_summary["smiles"] = smiles
        data_summary["curve_class"] = row["CURVE_CLASS2"]
        ...
        if curve_fit.successful_skewnorm_curvefit(a, loc, scale, ier):
            if hill_ac50_in_uM:
                plot_curves.plot_skewnorm_hill(
                    ...
                    hill_params["AC50"],
                    #    plot_file=None)
                    plot_file=f'{data_file}_{row["SAMPLE_DATA_ID"]}_both.jpg',
                )
            else:
        ...
        with open(
            f'{data_file}_{row["SAMPLE_DATA_ID"]}_both.json', "w", encoding="utf-8"
        ) as f:
